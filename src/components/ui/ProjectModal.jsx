...
export default function ProjectModal({ onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    name: "", domain: "health", budgetAllocated: "", startDate: "", endDate: "", description: ""
  });
  ...
  async function handleSubmit() {
    if (!form.name) return setError("Le nom du projet est requis.");
    if (!form.budgetAllocated || parseFloat(form.budgetAllocated) <= 0) return setError("Le budget alloué est requis.");
    setSaving(true);
    setError("");
    try {
      ...
      <div className={`w-full max-w-lg rounded-2xl border shadow-2xl ${dark ? "bg-emerald-900 border-emerald-800" : "bg-white border-emerald-100"}`}>
        ...
        <div className="px-6 py-5 grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className={labelClass}>{t.projects} *</label>
          ...
          <div>
            ...
            <input type="number" min="0" placeholder="0"
              value={form.budgetAllocated} onChange={e => set("budgetAllocated", e.target.value)}
              className={inputClass} />
          </div>
